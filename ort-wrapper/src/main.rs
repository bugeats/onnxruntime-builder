...
//!
//! Example: `RUST_LOG=debug ./ort-wrapper`
...
fn main() -> Result<()> {
    tracing_subscriber::registry()
        ...
        .with(EnvFilter::from_default_env().add_directive("ort_wrapper=info".parse()?))
        .init();
    ...
    info!("╔════════════════════════════════════════════════════════════════╗");
    info!("║              ONNX Runtime Test Harness                        ║");
    info!("╚════════════════════════════════════════════════════════════════╝");
    ...
    let model_path = match get_test_model_path() {
        ...
        Err(e) => {
            ...
            eprintln!();
            eprintln!("Usage:");
            eprintln!("  ONNX_TEST_MODEL=/path/to/model.onnx nix run .#ort-wrapper-cpu");
            eprintln!();
            eprintln!("Or with the bundled SqueezeNet model:");
            eprintln!("  ONNX_TEST_MODEL=$(nix build .#squeezenet-model --print-out-paths) nix run .#ort-wrapper-cpu");
            eprintln!();
            std::process::exit(1);
        }
    };
